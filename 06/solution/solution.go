...
outer:
	...
	columns := [][]string{}
	prev := 0
	for _, c := range column_indexes {
		chunks := []string{}
		for _, line := range lines {
			chunks = append(chunks, line[prev:c])
		}

		fmt.Println(chunks)
		chunklen := len(chunks[0])
		recomb := make([]string, len(chunks))
		for i := range chunklen {
			for _, c := range chunks {
				...
				if c[i] == '+' || c[i] == '*' {
					recomb = append(recomb, strings.Trim(c, " "))
					continue
				}
				recomb[i] = recomb[i] + string(c)
			}
		}
		columns = append(columns, recomb)
